...
FRONTEND       = True          # Set to False to run in terminal only
...
def run_pipeline(
    ...
    # ── Step 2.5: RGB damage area ─────────────────────────────────────────────
    if rgb_array is not None:
        print("Step 2.5 — RGB Damage Area...")
        rgb_result = analyze_rgb_damage(rgb_array, clf["subcategory"], backsheet_flag=backsheet_flag)
        clf["rgb_damage_percent"] = rgb_result.damage_percentage
    else:
        clf["rgb_damage_percent"] = None
    ...
    final = {
        ...
        "rgb_damage_percent": result.get("rgb_damage_percent"),
