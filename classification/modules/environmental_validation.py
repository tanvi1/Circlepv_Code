...
from modules.hko_correction     import get_hko_weather, process_measurement
from modules.iv_curve_simulator import simulate_iv_curve
...
def validate_environment(
    ...
    # ── HKO weather & Correction ──────────────────────────────────────────────
    if delta_t_proxy is not None:
        try:
            humidity, ambient_temp = get_hko_weather()
        except Exception as exc:
            logger.warning("HKO API failed: %s — using defaults", exc)
            humidity, ambient_temp = 70, 30

        try:
            correction = process_measurement(
                delta_t=delta_t_proxy, irradiance=irradiance,
                wind_speed=wind_speed, humidity=humidity, delta_p=iv.power_loss,
            )
        except Exception as exc:
        ...
        logger.info("delta_t_proxy missing — env correction skipped")
        humidity, ambient_temp, delta_t_corrected = None, None, None
        env_status = "SKIPPED"
