...
import os
from modules.backsheet import BacksheetDamage
...
# Circular Import Fix
from modules import client_algorithms

# def analyze_rgb_damage(image_input, class_name: str) -> DamageResult:
def analyze_rgb_damage(image_input, class_name: str, backsheet_flag: int = 1, client_mode: bool = False) -> DamageResult:
    """
    Universal entry point.
    ...
    if client_mode:
        client_result = client_algorithms.run_client_module(image, class_name)
        if client_result is not None:
            return client_result
        print(f"[INFO] No client module for {class_name}. Falling back to default.")
    ...
    if class_name == "BacksheetDamage" and backsheet_flag == 1:
        detector = BacksheetDamage()
        annotated, pct, dets = detector.detect(image)
        return DamageResult(
            class_name="BacksheetDamage",
            detected=len(dets) > 0,
            confidence=1.0 if len(dets) > 0 else 0.0,
            damage_percentage=pct,
            metadata={"detections": dets},
            annotated_image=annotated
        )

    # Note: Flag 0 is intentionally ignored/reserved for future use.
